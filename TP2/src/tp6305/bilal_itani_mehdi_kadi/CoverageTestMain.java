...
//		testRACC(coverageTest, 0.99);
	...
	private static void testBranchCoverage(CoverageTest coverageTest, double coverageThreshold) {
		...
		for (int i = 0; i < 3; i++) {
			...
			System.out.println("BranchCoverage is " + coverageTest.getBranchCoverage());
			System.out.println("Number of iteration is " + coverageTest.getIterationNum());
			total += coverageTest.getIterationNum();
		}

		System.out.println("To reach " + coverageThreshold * 100 + "% branch coverage, the average iteration number is "
				+ total / 3);
	}

	private static void testRACC(CoverageTest coverageTest, double coverageThreshold) {
		int total = 0;
		for (int i = 0; i < 3; i++) {

			coverageTest.testRACC(coverageThreshold);

			System.out.println("RACC is " + coverageTest.getRACC());
			System.out.println("Number of iteration is " + coverageTest.getIterationNum());
			total += coverageTest.getIterationNum();
		}

		System.out.println(
				"To reach " + coverageThreshold * 100 + "% RACC, the average iteration number is " + total / 3);
	}
